	...
	const app = {
	  ...
	  return this.products.filter((p) => !rg || p.descripcion.match(rg)); // Usar 'descripcion' en lugar de 'name'
		...
		async getClientBillingData() {
			const response = await fetch(localStorage.getItem('configCompanyApi') + "/buscar-cliente", {
				method: "POST",
				headers: {
					"Content-Type": "application/json",
				},
...
async function checkServer() {
	try {
		const url = `${localStorage.getItem('configCompanyApi')}/ver-habilitacion`;
		const payload = {
			pCodEmpresa: "1",
			pCodSucursal: "132",
			pCodCaja: "1",
...
function setStatePDV(val){
	if(val){
		...
		document.getElementById("tocaParaPedir").innerText = "Terminal fuera de servicio";
		document.getElementById("tocaParaPedirEn").innerText = "Terminal Out of Service";
...
function setDefaultConfig() {
	const defaults = {
		configBancardApi: "http://192.168.100.12:3000",
		...
		configPdvName: "PDV1",
		configProxyPos: 'http://192.168.100.2:4000'
	};
